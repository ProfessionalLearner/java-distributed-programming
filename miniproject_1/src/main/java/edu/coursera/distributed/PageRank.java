...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> updatedRanks = sites.join(ranks).values().flatMapToPair(kv -> {
            Website edges = kv._1();
            Double currRank = kv._2();

            List<Tuple2<Integer, Double>> cand = new LinkedList<Tuple2<Integer, Double>>();
            Iterator<Integer> iter = edges.edgeIterator();
           while(iter.hasNext()) {
               final int target = iter.next();
               cand.add(new Tuple2(target, currRank / edges.getNEdges()));
           }
